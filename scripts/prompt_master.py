...
_SYSTEM_TEMPLATE = "<BOS_TOKEN><|START_OF_TURN_TOKEN|><|SYSTEM_TOKEN|>\n{system_prompt}\n<|END_OF_TURN_TOKEN|>\n"
_CHAT_TEMPLATE = "<|START_OF_TURN_TOKEN|><|USER_TOKEN|>\n{query}\n<|END_OF_TURN_TOKEN|><|START_OF_TURN_TOKEN|><|CHATBOT_TOKEN|>"
_CHAT_TEMPLATE_END = "<|END_OF_TURN_TOKEN|>"
_PROMPT_PATH = "../prompts/svd_system_04.txt"
#_MODEL_PATH = "../models/dolphin-2.7-mixtral-8x7b.Q4_K_M.gguf"
_MODEL_PATH = "../models/c4ai-command-r-v01-Q5_K_M.gguf"
...
class SVDScanner(object):
    ...
    def local_tokenizer_encode(self, text: str) -> list[int]:
        return tokenizer.encode(text)
    
    def local_tokenizer_decode(self, token_ids: list[int]) -> str:
        return tokenizer.decode(token_ids)
    
    def local_tokenizer_length(text: str) -> int:
        return len(tokenize.encode(text))
    ...
    def prompt_splitter(self, inf_msg, system_msg, source):
        ...
        prompt_limit = _N_CTX - (2*_MAX_TOKENS) - limit_tokens
        ...
        prompts = text_splitter.split_text(source)
...
if __name__ == "__main__":
    ...
    system_prompt = system_f.read()

    scanner = SVDScanner(args)

    snippets = scanner.prompt_splitter(source, system_prompt, source)

    svd_results = []
    for snippet in snippets:
        query = f"Please summarize the following snippet of code:\n{snippet}"
        inf_msg = fstr(_SYSTEM_TEMPLATE) + fstr(_CHAT_TEMPLATE)
        query = f"Is there a security vulnerability in the snippet. Yes or no only."
        print(source)
        response = scanner.inference(inf_msg) + _CHAT_TEMPLATE_END + fstr(_CHAT_TEMPLATE)
        svd_result = scanner.inference(response)
    ...
    # # Trim to JSON format
    # svd_trimmed = svd_results[svd_results.find(_JSON_START) + len(_JSON_START) + 1: svd_results.rfind(_JSON_END)]
    # json_dict = json.loads(svd_trimmed)
    # # Remove all filtered non-vulnerabilities
    # final_results = [d for d in json_dict if d['vulnerability-type'].upper() not in _NON_VUL]

    # if not final_results:
    #     print("No vulnerabilities detected!")
    # else:
    #     json_object = json.dumps(final_results, indent=4)
